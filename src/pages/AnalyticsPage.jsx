...
const AnalyticsPage = ({ currentLanguage = 'ru' }) => {
  ...
  const loadAnalyticsData = async (useCache = true) => {
    try {
      ...
      toast({
        title: getLocaleString('analyticsError', currentLanguage),
        description: getLocaleString('analyticsErrorDesc', currentLanguage),
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleExportReport = () => {
    toast({
      title: getLocaleString('analyticsExportReport', currentLanguage),
      description: getLocaleString('analyticsExportReportDesc', currentLanguage),
    });
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold flex items-center gap-3">
                ...
                {getLocaleString('analyticsTitle', currentLanguage)}
              </h1>
              <p className="text-white/60 mt-1">
                {getLocaleString('analyticsSubtitle', currentLanguage)}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Select value={period} onValueChange={setPeriod}>
              ...
              <SelectContent>
                <SelectItem value="1d">
                  {getLocaleString('analyticsLast24Hours', currentLanguage)}
                </SelectItem>
                <SelectItem value="7d">
                  {getLocaleString('analyticsLast7Days', currentLanguage)}
                </SelectItem>
                <SelectItem value="30d">
                  {getLocaleString('analyticsLast30Days', currentLanguage)}
                </SelectItem>
              </SelectContent>
            </Select>
            ...
            >
              ...
              {getLocaleString('analyticsExport', currentLanguage)}
            </Button>
          </div>
        </div>
        ...
        {loading && !refreshing ? (
          <div className="flex items-center justify-center h-64">
            <div className="text-center">
              ...
              <p className="text-white/60">{getLocaleString('loading', currentLanguage)}</p>
            </div>
          </div>
        ) : (
          <>
            {/* Overview Stats */}
            <div className="mb-6">
              <AnalyticsOverview data={overviewData} currentLanguage={currentLanguage} />
            </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
              <TabsList className="bg-white/10 border-white/20">
                <TabsTrigger value="overview" className="data-[state=active]:bg-white/20">
                  {getLocaleString('analyticsOverview', currentLanguage)}
                </TabsTrigger>
                <TabsTrigger value="audience" className="data-[state=active]:bg-white/20">
                  {getLocaleString('analyticsAudience', currentLanguage)}
                </TabsTrigger>
                <TabsTrigger value="content" className="data-[state=active]:bg-white/20">
                  {getLocaleString('analyticsContent', currentLanguage)}
                </TabsTrigger>
                <TabsTrigger value="behavior" className="data-[state=active]:bg-white/20">
                  {getLocaleString('analyticsBehavior', currentLanguage)}
                </TabsTrigger>
              </TabsList>

              {/* Overview Tab */}
              <TabsContent value="overview" className="space-y-6">
                <VisitorsChart data={visitorsData} currentLanguage={currentLanguage} />
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <TrafficSources data={trafficSourcesData} currentLanguage={currentLanguage} />
                  <DevicesBreakdown data={devicesData} currentLanguage={currentLanguage} />
                </div>
              </TabsContent>
              ...
              <TabsContent value="audience" className="space-y-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <GeographyMap data={geographyData} currentLanguage={currentLanguage} />
                  <DevicesBreakdown data={devicesData} currentLanguage={currentLanguage} />
                </div>
                <TrafficSources data={trafficSourcesData} currentLanguage={currentLanguage} />
              </TabsContent>
              ...
              <TabsContent value="content" className="space-y-6">
                <PopularEpisodesChart data={popularEpisodes} currentLanguage={currentLanguage} />
                <ListeningStats data={listeningStatsData} currentLanguage={currentLanguage} />
              </TabsContent>

              {/* Behavior Tab */}
              <TabsContent value="behavior" className="space-y-6">
                <BehaviorMetrics data={behaviorMetricsData} currentLanguage={currentLanguage} />
                <VisitorsChart data={visitorsData} currentLanguage={currentLanguage} />
              </TabsContent>
            </Tabs>
          </>
        ...
        <div className="mt-8 p-4 bg-white/5 rounded-lg border border-white/10">
          <p className="text-white/60 text-sm text-center">
            {getLocaleString('analyticsFooterNote', currentLanguage)}
          </p>
        </div>
      </div>
    </div>
