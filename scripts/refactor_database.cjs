...
const dbClient = new Client({
  host: 'supabase.dosmundos.pe',
  port: 5432,
  user: 'postgres',
  password: POSTGRES_PASSWORD,
  database: 'postgres',
  ssl: false // Disable SSL for self-hosted instance
});
...
async function executeSQL(sql) {
  return new Promise((resolve, reject) => {
    // Try using the Supabase RPC endpoint for executing SQL
...
async function main() {
  console.log('Starting automated database refactor migration...');
  ...
  // Step 2: Fetch all existing articles
